...
4. **Build foundation** — 🟡 *scaffold selesai 2026-06-14*: skeleton **CMake** + shim tipe Win32 (`platform/win32_compat.h`) + dev container Linux, dengan target gabungan `foundation_smoke` (tipe + `boost::asio` + ODBC dalam satu proyek) **compile/link/run hijau**. Gate verifikasi = **`make verify` lokal** (tanpa hosted CI — A2 cloud-exit). Lihat [runbook](runbooks/build-foundation.md).
