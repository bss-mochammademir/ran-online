...
> ✅ **Praktik wajib (standing rule) — rekomendasi model bersifat *forward-looking*.** Berikan rekomendasi saat **menutup** sebuah tugas & hendak lanjut: tutup dengan satu baris untuk **tugas berikutnya** — *"▶️ Tugas berikutnya (`<deskripsi>`) direkomendasikan: `<model>` @ `<effort>` — `<alasan>`"* — lalu **beri jeda agar manusia sempat mengganti model sebelum tugas itu dijalankan**. **Jangan** menaruh rekomendasi di awal tugas yang sedang berjalan: saat itu modelnya sudah terlanjur aktif → tak ada *window* untuk switch. Field **Model & effort** tetap wajib diisi di chip-spec ([§3](#3-anatomi-sebuah-chip)) saat mendekomposisi (juga pra-eksekusi). Tujuannya: keputusan kapabilitas/biaya jadi **eksplisit, dapat diaudit (A7), dan *dapat ditindaklanjuti*** — manusia punya kesempatan switch/delegasi **sebelum** biaya keluar.
