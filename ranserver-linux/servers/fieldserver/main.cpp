...
int main(int argc, char** argv) {
    ...
    if (port == 0) {
        const char* p = std::getenv("FIELD_PORT");
        port = p ? static_cast<unsigned short>(std::atoi(p)) : 9501;
    }
    ...
    if (srv.Start() != 0) { std::cerr << "FieldServer failed to start\n"; return 1; }
    std::cerr << "FieldServer running on port " << port << " (SIGINT/SIGTERM to stop)\n";
