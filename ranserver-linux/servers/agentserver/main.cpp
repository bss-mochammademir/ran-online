...
int main(int argc, char** argv) {
    ...
    if (port == 0) {
        const char* p = std::getenv("AGENT_PORT");
        port = p ? static_cast<unsigned short>(std::atoi(p)) : 9301;
    }
    ...
    if (srv.Start() != 0) { std::cerr << "AgentServer failed to start\n"; return 1; }
    std::cerr << "AgentServer running on port " << port << " (SIGINT/SIGTERM to stop)\n";
