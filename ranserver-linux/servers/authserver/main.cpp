// AuthServer headless entry point — the cross-platform replacement for the Win32
// WinMain + MFC dialog (AuthServer/AuthServer.cpp): the dialog's Start/Stop buttons
// and tray icon become process start + SIGINT/SIGTERM -> graceful Stop(). This is
// the fan-out template every ported server's main() follows.
...
int main(int argc, char** argv) {
    ...
    if (port == 0) {
        const char* p = std::getenv("AUTH_PORT");
        port = p ? static_cast<unsigned short>(std::atoi(p)) : 9101;
    }
    ...
    if (srv.Start() != 0) { std::cerr << "AuthServer failed to start\n"; return 1; }
    std::cerr << "AuthServer running on port " << port << " (SIGINT/SIGTERM to stop)\n";
