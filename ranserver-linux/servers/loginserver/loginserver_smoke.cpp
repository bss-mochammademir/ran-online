...
int main() {
    ...
    const unsigned short port = 18902;
    std::string dbConn;
    ...
    std::cout << "LoginServer smoke — lifecycle + framed packet pipeline.\n";
    sc::servers::LoginServer srv(port, dbConn);
    ...
    bool connected = false;
    {
        ...
        connected = !ec;
        if (connected) {
            auto m1 = EncodeMessage(101, "lobby", 5);
            auto m2 = EncodeMessage(202, "channel", 7);
    ...
    if (echoes.size() == 3) {
        check("echo#1 type+payload", echoes[0].type == 101 &&
              std::string(echoes[0].payload(), echoes[0].payloadSize()) == "lobby");
        check("echo#2 type+payload (split)", echoes[1].type == 202 &&
              std::string(echoes[1].payload(), echoes[1].payloadSize()) == "channel");
    ...
    std::cout << "\n";
    if (g_fail == 0) {
        std::cout << "LOGINSERVER SMOKE OK: asio lifecycle + framed packet pipeline"
                  << (dbConn.empty() ? "" : " + OdbcDb connect") << ".\n";
        return 0;
    }
    std::cout << "LOGINSERVER SMOKE FAILED: " << g_fail << " assertion(s).\n";
    return 1;
}
