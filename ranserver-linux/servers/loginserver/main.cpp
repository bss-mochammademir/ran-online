...
int main(int argc, char** argv) {
    ...
    if (port == 0) {
        ...
        port = p ? static_cast<unsigned short>(std::atoi(p)) : 9200;
    }
    ...
    if (srv.Start() != 0) { std::cerr << "LoginServer failed to start\n"; return 1; }
    std::cerr << "LoginServer running on port " << port << " (SIGINT/SIGTERM to stop)\n";
