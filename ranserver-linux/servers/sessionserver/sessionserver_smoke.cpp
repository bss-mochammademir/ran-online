...
int main() {
    ...
    const unsigned short port = 18903;
    std::string dbConn;
    ...
    std::cout << "SessionServer smoke — lifecycle + framed packet pipeline.\n";
    ...
    bool connected = false;
    {
        ...
        connected = !ec;
        if (connected) {
            auto m1 = EncodeMessage(101, "sync", 4);
            auto m2 = EncodeMessage(202, "chatrelay", 9);
            auto m3 = EncodeMessage(303, nullptr, 0);
            asio::write(s, asio::buffer(m1), ec);
            asio::write(s, asio::buffer(m2.data(), 6), ec);
            asio::write(s, asio::buffer(m2.data() + 6, m2.size() - 6), ec);
    ...
    if (echoes.size() == 3) {
        check("echo#1 type+payload", echoes[0].type == 101 &&
              std::string(echoes[0].payload(), echoes[0].payloadSize()) == "sync");
        check("echo#2 type+payload (split)", echoes[1].type == 202 &&
              std::string(echoes[1].payload(), echoes[1].payloadSize()) == "chatrelay");
    ...
    std::cout << "\n";
    if (g_fail == 0) {
        std::cout << "SESSIONSERVER SMOKE OK: asio lifecycle + framed packet pipeline"
                  << (dbConn.empty() ? "" : " + OdbcDb connect") << ".\n";
        return 0;
    }
    std::cout << "SESSIONSERVER SMOKE FAILED: " << g_fail << " assertion(s).\n";
    return 1;
}
